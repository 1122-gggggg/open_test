...
export default function CurriculumSidebar({ subjects, selectedChapterId, onSelect }: {
  ...
  return (
    <div className="bg-white border rounded-xl p-3 space-y-2">
      <h3 className="font-bold text-sm px-1">108 課綱章節</h3>
      {categories.map(cat => (
        <div key={cat} className="border rounded-lg overflow-hidden">
          <button onClick={()=> setOpenCats(o=> ({...o, [cat]: !o[cat]}))} className="w-full flex justify-between items-center px-3 py-2 bg-slate-50 text-sm font-medium">
            <span>{cat}</span><span className="text-xs text-slate-400">{openCats[cat] ? "−" : "+"}</span>
          </button>
          {openCats[cat] && (
            <div className="divide-y">
              {subjects.filter(s=> s.category===cat).map(sub => {
                ...
                return (
                  <div key={sub.id}>
                    <button onClick={()=> setOpenSubjects(o=> ({...o, [sub.code]: !isOpen}))} className="w-full flex justify-between items-center px-3 py-2 text-sm hover:bg-slate-50">
                      <span className="flex items-center gap-2"><span>{sub.icon}</span>{sub.name}</span>
                      <span className="text-xs text-slate-400">{sub.chapters.length} 章</span>
                    </button>
                    {isOpen && (
